...
type Screen =
  ...
  | "google-phone"   // Google login: collect phone
  | "google-otp";    // Google login: enter OTP for phone
...
export default function LoginPage() {
  ...
  async function handleSignupSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await auth.patientSignup(name.trim(), email.trim().toLowerCase(), password, phone.trim());
      setOtpId(res.otpId); setMaskedPhone(res.maskedPhone);
      setScreen("otp"); startResendTimer();
  ...
  async function handleVerifyOTP(e: React.FormEvent) {
    ...
    try {
      ...
      login(user, token);
      toast.success(screen === "otp" && patientMode === "signup"
        ? "Account created! Welcome to Doctor Booked."
        : "Welcome back!");
      navigate({ path: "/patient/hospitals" });
  ...
  // ── Google OTP verify ─────────────────────────────────────────────────────────
  async function handleGoogleOTPVerify(e: React.FormEvent) {
    e.preventDefault();
    if (otp.trim().length !== 6) { toast.error("Enter the 6-digit OTP"); return; }
    setLoading(true);
    try {
  ...
  async function handleGooglePhoneSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!googlePhone.trim()) { toast.error("Enter your phone number"); return; }
    setLoading(true);
    try {
      const res = await auth.googlePhoneOTP(googleUserId, googlePhone.trim());
      setOtpId(res.otpId); setMaskedPhone(res.maskedPhone);
      setScreen("google-otp"); startResendTimer();
      if (res.devOtp) toast.info(`[Dev] OTP: ${res.devOtp}`, { duration: 60000 });
      else toast.success(res.message);
    } catch (err: any) { toast.error(err.message || "Failed to send OTP"); }
    finally { setLoading(false); }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="w-full max-w-sm bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          ...
          <form onSubmit={screen === "google-otp" ? handleGoogleOTPVerify : handleVerifyOTP} className="space-y-4">
            ...
            <button type="button" className="w-full text-xs text-gray-400 hover:text-gray-600 text-center"
              onClick={() => { setScreen(screen === "google-otp" ? "google-phone" : "patient-form"); setOtp(""); }}>
              ← Back
            </button>
          </form>
        </div>
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="w-full max-w-sm bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-gray-500 mt-2">Add your phone for appointment reminders and account security.</p>
          </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input type="tel" className="pl-9" placeholder="9876543210"
                  value={googlePhone} onChange={e => setGooglePhone(e.target.value)} autoFocus />
              </div>
